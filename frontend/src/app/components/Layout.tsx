...
export function Layout() {
  ...
  return (
    <div className="flex h-screen min-h-screen flex-col overflow-x-hidden bg-slate-50 md:flex-row">
      {/* Sidebar / Topbar */}
      <aside
      ...
      >
        {/* Logo & Toggle */}
        <div
          className={`p-4 flex items-center justify-between border-b border-slate-700 md:border-transparent ${!sidebarOpen ? "md:justify-center" : ""}`}
        >
          <div className={`${!sidebarOpen ? "md:hidden" : ""}`}>
            <ProjectLogo
              ...
              subtitleClassName="text-sm font-bold text-white"
              subtitle="Klabin"
        ...
        {/* Navigation & Profile */}
        <div
          className={`flex-1 flex-col ${!sidebarOpen ? "hidden md:flex" : "flex"}`}
        >
            ...
            {menuItems.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  <span
                    className={`font-medium text-sm whitespace-nowrap ${!sidebarOpen ? "md:hidden" : ""}`}
                  >
                    {item.label}
                  </span>
                </Link>
          ...
          {/* User Profile */}
          <div
            className={`p-4 border-t border-slate-700 flex flex-col gap-4 ${!sidebarOpen ? "md:items-center" : ""}`}
          >
            <div
            ...
            >
              ...
              <div
                className={`space-y-0.5 min-w-0 ${!sidebarOpen ? "md:hidden" : ""}`}
              >
            ...
            <button
              ...
              className={`inline-flex items-center justify-center gap-2 rounded-xl border border-slate-700 bg-slate-900 font-medium text-slate-300 hover:bg-slate-700/50 hover:text-blue-400 transition-colors ${
                !sidebarOpen
                  ? "w-10 h-10 p-0 rounded-full md:rounded-xl"
                  : "w-full px-3 py-2 text-sm"
              }`}
              title="Sair do sistema"
            >
              <LogOut className="w-4 h-4 shrink-0" />
              <span className={`${!sidebarOpen ? "hidden" : "inline"}`}>
                Logout
              </span>
            </button>
          </div>
      ...
      {/* Main Content */}
      <main className="min-w-0 flex-1 flex h-full min-h-0 flex-col overflow-hidden">
        {/* Top Bar */}
