...
export function Login() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(
        err.message ||
          "Erro ao conectar ao servidor. Verifique as suas credenciais.",
      );
    } finally {
      setIsLoading(false);
    }
    ...
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
      <div className="max-w-md w-full bg-white rounded-2xl shadow-xl p-8 border border-slate-100">
        <header className="flex flex-col items-center mb-8">
          <ProjectLogo compact iconClassName="h-16 w-16 mb-4" />
          <h1 className="text-2xl font-bold text-slate-800">SAP Script AI</h1>
          <p className="text-slate-500 text-sm mt-1">
            Insira as suas credenciais Klabin
          </p>
        </header>
        ...
        <footer className="mt-6 text-center text-sm text-slate-600">
          ...
          <Link
            to="/signup"
            className="text-blue-600 font-medium hover:underline focus:ring-2 focus:ring-blue-400 rounded p-1"
          >
            Registe-se aqui
          </Link>
        </footer>
